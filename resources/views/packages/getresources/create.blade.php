...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('getresource.setgroup') }}" enctype="multipart/form-data">
            @csrf
            <!-- category -->
            ...
            <div class="form-group row">
                ...
                <div class="col-md-4">
                    <select id="grp" class="form-control @error('grp') is-invalid @enderror" name="grp" value="{{ old('grp') }}" required autocomplete="grp" autofocus>
                        ...
                        <option value="{{$group->id}}">{{$group->room_name}}</option>
                        @endforeach
                        @else
                        <div class="alert alert-info">No groups <a href="{{route('category.create')}}" class="btn btn-sm btn-primary pull-right"><i class="fa fa-plus"> </i> Add </div>
                        @endif
                    </select>
                    @error('grp')
                    <span class="invalid-feedback" role="alert">
            ...
            <div class="form-group row mb-0">
                <div class="col-md-8 offset-md-4">
                    <button type="submit" class="btn btn-success">
                        <i class="fa fa-check"></i> {{ __('Set Group') }}
                    </button>
